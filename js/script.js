(() => {
  ...
  media.add('(prefers-reduced-motion: no-preference)', () => {
    ...
    intro.fromTo('.hero-top > *', { y: 12, opacity: 0 }, { y: 0, opacity: 1, duration: .8, stagger: .12 })
      .fromTo('.hero h1 > span', { y: () => small() ? 20 : 36, opacity: 0 }, { y: 0, opacity: 1, duration: 1.05, stagger: .16 }, .15)
      .fromTo('.hero-skills > span', { y: 9, opacity: 0 }, { y: 0, opacity: 1, duration: .65, stagger: .055 }, .55)
      .fromTo('.orb-scene', { scale: .94, opacity: 0 }, { scale: 1, opacity: 1, duration: 1.1 }, .25)
      .fromTo('.hero-bottom > *', { y: 15, opacity: 0 }, { y: 0, opacity: 1, duration: .85, stagger: .12 }, .7);
    ...
    sectionTransition
      ...
      .fromTo('.hero-title', { y: 0, scale: 1, opacity: 1, filter: 'blur(0px)' }, {
        y: () => small() ? -12 : -28,
        scale: () => small() ? .99 : .97,
        opacity: .48,
        filter: () => small() ? 'blur(1px)' : 'blur(2px)',
    ...
    const measureStar = () => {
      ...
      starStart = {
        x: art.right + window.scrollX - flyingStar.offsetWidth / 2,
